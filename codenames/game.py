...
class Game:
    ...
    def __init__(self, codemaster, guesser,
        ...
        with open("game_wordpool.txt", "r") as f:
            ...
            random.shuffle(temp)      # Derek change
            self.words_on_board = temp[:25]

        # set grid key for codemaster (spymaster)
        with open("game_wordKey.txt", "r") as f:
            temp = f.read().split()
            self.temp_key_grid = temp[:25]
        #self.temp2_key_grid = self.temp_key_grid
        #self.temp2_key_grid = []
        self.key_grid = []
        for i in self.temp_key_grid[:25]:
            if i == "a":
                #i = "Assassin"
                self.key_grid.append("Assassin")
            elif i == "c":
                i = "Civ"
                self.key_grid.append("Civilian")
            elif i == "b":
                i = "Blue"
                self.key_grid.append("Blue")
            elif i == "r":
                i = "Red"
                self.key_grid.append("Red")
            else:
                self.key_grid.append(" ")
                i = i
    ...
    def run(self):
        ...
        while game_condition != GameCondition.LOSS and game_condition != GameCondition.WIN:
            ...
            clue, clue_num, agent = self.codemaster.get_clue()
            game_counter += 1
            keep_guessing = True
            guess_num = 0
            ...
            while guess_num <= clue_num and keep_guessing and game_condition == GameCondition.HIT_RED:
                ...
                guess_answer, ordered_words = self.guesser.get_answer()
                ...
                with open("first_guess_log.txt", "a") as a_file:
                    a_file.write("\n")
                    # a_file.write("Clue, " + clue + ", Guess, " + guess_answer)
                    a_file.write(
                        "Agent: " + agent[0] + ", Clue: " + clue + ' ')
                    a_file.write(f'{ordered_words}')
                    # for word in self.get_words_on_board():
                    #     a_file.write(", " + word)

    # uncomcoment all this to resume to normal gameplay
        # while game_condition != GameCondition.LOSS and game_condition != GameCondition.WIN:
        #     # board setup and display
        #     print('\n' * 2)
        #     words_in_play = self.get_words_on_board()
        #     current_key_grid = self.get_key_grid()
        #     self.codemaster.set_game_state(words_in_play, current_key_grid)
        #     # self._display_key_grid()   # Uncomment to show grid, sorta useless because it's color coordinated
        #     self._display_board_codemaster()
